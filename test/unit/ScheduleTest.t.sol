...
contract ScheduleTest is DcaOutTestBase {
    ...
    function testCannotCreateDcaOutScheduleWithAmountTooSmall() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDcaOutManager.DcaOutManager__SaleAmountBelowMinimum.selector, MIN_SALE_AMOUNT - 1, MIN_SALE_AMOUNT
            )
        );
    ...
    function testCannotCreateDcaOutScheduleWithPeriodTooShort() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDcaOutManager.DcaOutManager__SalePeriodBelowMinimum.selector, MIN_SALE_PERIOD - 1, MIN_SALE_PERIOD
            )
        );
    ...
    function testRevertDepositRbtcWithInvalidScheduleId() public {
        ...
        bytes memory encodedRevert = abi.encodeWithSelector(
            IDcaOutManager.DcaOutManager__ScheduleIdAndIndexMismatch.selector, wrongScheduleId, schedule.scheduleId
        );
        vm.expectRevert(encodedRevert);
        vm.prank(user);
    ...
    function testWithdrawRbtc() public {
        ...
        assertEq(
            dcaOutManager.getScheduleRbtcBalance(user, 0), DEPOSIT_AMOUNT / 2, "Schedule should have remaining balance"
        );
    }
    ...
    function testWithdrawRbtcMoreThanBalanceWithdrawsAll() public {
        ...
        assertEq(
            balanceAfter - balanceBefore, DEPOSIT_AMOUNT, "User should receive all deposited rBTC back (and no more)"
        );
    ...
    function testCannotSetSalePeriodTooShort() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDcaOutManager.DcaOutManager__SalePeriodBelowMinimum.selector, invalidSalePeriod, MIN_SALE_PERIOD
            )
        );
        vm.prank(user);
